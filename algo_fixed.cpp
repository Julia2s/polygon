...
vector<Node*> dfs(Graph& graph, Node* start, Node* goal) {
    stack<pair<Node*, double>> s; // Память: O(V) для стека
    map<Node*, Node*> parent; // Память: O(V) для хранения пути
...
double calculate_path_weight(const vector<Node*>& path) {
    double weight = 0;
    for (size_t i = 0; i < path.size() - 1; ++i) {
        for (const auto& [neighbor, w] : path[i]->neighbors) {
...
void test_algorithms(Graph& graph, Node* start, Node* goal, double expected_weight) {
    auto bfs_path = bfs(graph, start, goal);
    double bfs_weight = calculate_path_weight(bfs_path);
    cout << "BFS Path Weight: " << bfs_weight << endl;
    assert(bfs_weight == expected_weight); // Ожидаемый вес пути

    auto dfs_path = dfs(graph, start, goal);
    double dfs_weight = calculate_path_weight(dfs_path);
    cout << "DFS Path Weight: " << dfs_weight << endl;
    assert(dfs_weight == expected_weight); // Ожидаемый вес пути

    auto dijkstra_path = dijkstra(graph, start, goal);
    double dijkstra_weight = calculate_path_weight(dijkstra_path);
    cout << "Dijkstra Path Weight: " << dijkstra_weight << endl;
    assert(dijkstra_weight == expected_weight); // Ожидаемый вес пути

    auto a_star_path = a_star(graph, start, goal);
    double a_star_weight = calculate_path_weight(a_star_path);
    cout << "A* Path Weight: " << a_star_weight << endl;
    assert(a_star_weight == expected_weight); // Ожидаемый вес пути

    cout << "All tests passed for path from (" << start->lat << ", " << start->lon << ") to ("
         << goal->lat << ", " << goal->lon << ") with expected weight " << expected_weight << "!" << endl;
}

int main() {
    
    Graph test_graph;
    test_graph.load_from_file("test.txt");

    Node* test_start_node = test_graph.find_closest_node(1.0, 1.0);
    Node* test_goal_node = test_graph.find_closest_node(7.0, 7.0);

    test_algorithms(test_graph, test_start_node, test_goal_node, 3.0);

    Graph spb_graph;
    spb_graph.load_from_file("spb_graph.txt");

    Node* spb_start_node = spb_graph.find_closest_node(59.867173, 30.307543);
    Node* spb_goal_node = spb_graph.find_closest_node(30.309988, 30.308108);

    auto start_time = high_resolution_clock::now();
    auto bfs_path = bfs(spb_graph, spb_start_node, spb_goal_node);
    auto end_time = high_resolution_clock::now();
    cout << "BFS Time: " << duration<double>(end_time - start_time).count() << " seconds\n";

    start_time = high_resolution_clock::now();
    auto dfs_path = dfs(spb_graph, spb_start_node, spb_goal_node);
    end_time = high_resolution_clock::now();
    cout << "DFS Time: " << duration<double>(end_time - start_time).count() << " seconds\n";

    start_time = high_resolution_clock::now();
    auto dijkstra_path = dijkstra(spb_graph, spb_start_node, spb_goal_node);
    end_time = high_resolution_clock::now();
    cout << "Dijkstra Time: " << duration<double>(end_time - start_time).count() << " seconds\n";

    start_time = high_resolution_clock::now();
    auto a_star_path = a_star(spb_graph, spb_start_node, spb_goal_node);
    end_time = high_resolution_clock::now();
